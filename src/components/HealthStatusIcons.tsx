...
const HealthStatusIcons = () => {
  ...
  // Dynamically assign color based on healthPercentage
  const getColor = (healthPercentage: number): string => {
    if (healthPercentage >= 100) {
      return 'green';
    } else if (healthPercentage >= 10) {
      // Green to Amber/Orange range
      const redValue = Math.round(255 * ((100 - healthPercentage) / 90)); // Red increases as percentage decreases
      const greenValue = Math.round(255 * ((healthPercentage - 10) / 90)); // Green decreases as percentage decreases
  ...
  useEffect(() => {
    if (svgRef.current) {
      gsap.to(svgRef.current, {
        fill: getColor(healthPercentage),
        duration: 0.5,
        ease: "power2.inOut"
      })
    }
  ...
  useEffect(() => {
    if (rotationRef.current) {
      gsap.to(rotationRef.current, {
        rotationY: 360, // Full rotation on Y axis
        duration: 10,
  ...
  return (
    <div className="grid grid-cols-12 gap-4">
      ...
      <div className="p-2 col-span-8 lg:col-span-9 flex">
        <div className="flex flex-col lg:flex-row lg:items-start justify-start text-start w-full">
          <div className="lg:pr-4">
            <p className="text-sm text-muted-foreground">
              Your sleep has improved. Consider more physical activity.
            </p>
  
            {/* Health Score Input */}
            <div className="flex items-center mt-4 space-x-4 lg:whitespace-nowrap">
              <h2>Health Score</h2> 
              <input
                type="number"
      ...
      {/* Human Body Icon Section */}
      <div className="col-span-4 lg:col-span-3 flex lg:flex-col lg:items-end justify-center lg:justify-end">
        {/* Wrapper with perspective */}
        <div ref={rotationRef} style={{ perspective: '1000px' }}>
          <HumanBodyIcon color={getColor(healthPercentage)} ref={svgRef} />
        </div>
      </div>
    </div>
  )
...
// SVG component with dynamic color
const HumanBodyIcon = React.forwardRef<SVGSVGElement, HumanBodyIconProps>(({ color }, ref) => (
  <svg
    ref={ref}
    fill={color}
    version="1.1"
...
HumanBodyIcon.displayName = 'HumanBodyIcon'

export default HealthStatusIcons